...
class CHESSTeamBuilder:
    def __init__(self, config: Dict[str, any]) -> None:
        ...
        self.config = config
        logging.info("Initialized TeamBuilder with {state_class.__name__}") 
    ...
    def build(self):
        ...
        # Skip adding evaluation node
        # self.team.add_node("evaluation", ExecutionAccuracy())
        
        # Connect agents without evaluation
        agents_list = list(agents.keys())
        self.team.set_entry_point(agents_list[0])
        connections = [(agents_list[i], agents_list[i+1]) 
                       for i in range(len(agents_list)-1)]
        connections += [(agents_list[-1], END)]
